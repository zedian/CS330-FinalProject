...
parser = argparse.ArgumentParser(
                prog = 'Roberta Model',
                description = 'Experiments on the RoBERTa model')
parser.add_argument('mode', help=f"Execution type {','.join(PERMITTED_MODE)}")
parser.add_argument('-s', '--source', type=str,
                    default='roberta-base',
                    help=f"Source of roberta model: {','.join(SOURCES)}")
...
backbone, tokenizer = language_models.get_model_and_tokenizer(args.source)
...
if mode == 'grad':
    modelShared = language_models.TaskAwareRoberta(backbone, list(tasks.keys()),
                                                topology='shared',
    ...
    # Put the most heavy model first
    topIterant = ['separate', 'surgical', 'shared'] \
    ...
    for topology in topIterant:
        ...
        model = language_models.TaskAwareRoberta(backbone, list(tasks.keys()),
                                                topology=topology,
                                                source=args.source,
                                                sharing=param_keys)\
                               .to(args.device)
